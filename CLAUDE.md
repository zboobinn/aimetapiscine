...
- Depuis un Client Component, ne JAMAIS importer depuis le barrel
  `@/components/nuancier` : il réexporte `PriceBlock` → `service-role.ts`
  (`server-only`), ce qui casse `next build`. Toujours viser le module concret
  (`@/components/nuancier/swatch-group`).
- Exception route `/` (registre ArmaPool, cf. `docs/decisions.md` 2026-07-19) :
  sur cette route uniquement, les invariants ombre portée / `--radius: 2px` /
  mono-sur-nombres ci-dessus sont amendés, et `--turquoise` vaut `#08b9d4`
  (bleu du logo, utilisé comme accent d'UI). Le reste du site garde les
  invariants d'origine ci-dessus tels quels.
